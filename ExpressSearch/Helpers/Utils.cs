...
namespace ExpressSearch.Helpers
{
    public class Utils
    {
        ...
        /// <param name="str">要转换的字符串</param>
        /// <returns>转换后的int类型结果</returns>
        public static DateTime? StrToDateTimeOrNull(string str)
        {
            if (string.IsNullOrEmpty(str))
            {
                return null;
        ...
        #region 手机APP内容 清除HTML标记
        public static string AppDropHTML(string Htmlstring)
        {
            //  Htmlstring = ToTxt(Htmlstring);
            if (string.IsNullOrEmpty(Htmlstring)) return "";
            //删除脚本  
            Htmlstring = Regex.Replace(Htmlstring, @"<script[^>]*?>.*?</script>", "", RegexOptions.IgnoreCase);
            //删除HTML  
            ...
            Htmlstring = Regex.Replace(Htmlstring, @"&#(\d+);", "", RegexOptions.IgnoreCase);
            Htmlstring = Regex.Replace(Htmlstring, @"\\", "-", RegexOptions.IgnoreCase);
            Htmlstring = Regex.Replace(Htmlstring, "\"", "'", RegexOptions.IgnoreCase);//去除 " 符号
            Htmlstring = Regex.Replace(Htmlstring, @"\\r", "<r>", RegexOptions.IgnoreCase);
            Htmlstring = Regex.Replace(Htmlstring, @"\\n", "<n>", RegexOptions.IgnoreCase);
            //Htmlstring = Regex.Replace(Htmlstring, @" ", "", RegexOptions.IgnoreCase);//替换空字符串
            //Htmlstring = HttpContext.Current.Server.HtmlEncode(Htmlstring).Trim();
            Htmlstring = NoHTML(Htmlstring);
            return Htmlstring;
        }
        public static string AppDropHTMLNew(string Htmlstring)
        {
            //  Htmlstring = ToTxt(Htmlstring);
            if (string.IsNullOrEmpty(Htmlstring)) return "";
            ...
            //Htmlstring = Regex.Replace(Htmlstring, @" ", "", RegexOptions.IgnoreCase);//替换空字符串
            //Htmlstring = HttpContext.Current.Server.HtmlEncode(Htmlstring).Trim();
            return System.Web.HttpUtility.UrlEncode(Htmlstring, System.Text.Encoding.UTF8);
        }
        ///   <summary>   
        ///    去除HTML标记   
        ///   </summary>   
        ...
        public static string GetMapPath(string strPath)
        {
            ...
            {
                //TicketStystem票务系统共用一个配置文件
                //string mapPath = HttpContext.Current.Server.MapPath(strPath);
                //if (mapPath.ToLower().Contains("ticketsystem"))
                //{
                //    return mapPath.Replace("TicketSystem", "DTcms.Web");
                //}
                //return mapPath;
        ...
        public static void WriteCookie(string strName, string key, string strValue)
        {
            ...
            cookie.Domain = HttpContext.Current.Request.Url.Authority.ToLower();
            HttpContext.Current.Response.AppendCookie(cookie);
        }
        ...
        {
            string appkey = "aec80c85d638a0e3b03c4ff61b3b6ee1";

            //POST 传值
            byte[] bData = System.Text.Encoding.GetEncoding("UTF-8").GetBytes(param);
            ...
            //JsonData newObj = JsonMapper.ToJson(postReturn);
            //string errorCode = newObj["error_code"].ToString();
        ...
        public static string HttpGet(string url)
        {
            ...
            {
                //log4net 记录异常
                // DTcms.Web.Code.SiteHelper.PrintError(ex);
            }
            finally
            {
                request = null;
        ...
        /// <returns></returns>
        public static string HttpPostBinaryData(string url, byte[] bytes)
        {
        ...
        /// 内容图片增加网址
        /// </summary>
        public static string AddContentImgURL(string content)
        {
            Regex reg = new Regex(@"<img\s+[^>]*\s*src\s*=\s*([']?)(?<url>\S+)'?[^>]*>", RegexOptions.IgnoreCase);
            string str = "";
            MatchCollection mc = reg.Matches(content);
            foreach (Match match in mc)
            {
                str = match.Groups["url"].Value.ToString();
                if (str.IndexOf("http://" + HttpContext.Current.Request.Url.Host) == -1)
                    content = content.Replace(str, str.Replace("/upload/", "http://" + HttpContext.Current.Request.Url.Host + "/upload/"));
            }
            return content;
        }
        #endregion
        ...
        public static DateTime RandomTime(DateTime startTime, DateTime endTime)
        {
            Random rd = new Random();
            TimeSpan tsp = startTime - endTime;
            int days = rd.Next(0, tsp.Days);
            int hours = rd.Next(0, tsp.Days);
            int minu = rd.Next(0, tsp.Days);
            DateTime newtime = endTime.AddDays(days).AddHours(hours).AddMinutes(minu).AddSeconds(0);
            return newtime;
        }
        #endregion
        ...
        /// 返回上传目录相对路径
        /// </summary>
        /// <param name="saveType">创建文件夹命名类型</param>
        public static string GetUpLoadPath(int saveType)
        {
        ...
        /// 图片转换成字节
        /// </summary>
        /// <param name="imagePath">图片的相对路径</param>
        /// <returns></returns>
